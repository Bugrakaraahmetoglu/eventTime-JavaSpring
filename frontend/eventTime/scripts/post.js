document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('post-form').addEventListener('submit', async (event) => {
        ...
        const eventTime = document.getElementById('eventTime').value; // YYYY-MM-DD formatında olacak
        const image = document.getElementById('image').value;
        const cityId = document.getElementById('city').value;

        // Kullanıcının ID'sini sessionStorage'dan al
        ...
        const requestBody = {
            userId: userId,
            content: content,
            eventTime: eventTime, // Doğru formatta gönderiliyor
            image: image || null,
            cityId: cityId
        };
        ...
        try {
            // API isteği
            const response = await fetch('http://127.0.0.1:8080/api/post/addPost', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                alert('Post oluşturulurken bir hata oluştu: ' + errorData.message);
            }
        } catch (error) {
